...
5. **Inflate** leftover straights and curves as detours. Nested switch/crossover pieces (`rte` / `xo` / `par` / `cr` / `kel`) only get S-bends. `ovalJoin` on those circuits bridges two nearby diverges into a tiny bypass. On a large closed core, a leftover two-curve passing close is rejected so the second route stays at least six extra pieces. Later passes may inflate the main loop.
